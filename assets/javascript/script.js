...
var primaryDoc = false;

// var queryBetterDoctor = "https://api.betterdoctor.com/2016-03-01/doctors?location=35.732,-78.850,10&limit=10&user_key=c570e2001f75afa94675595ad2391528";
...
function doctorList(queryBetterDoctor) {
    ...
    return $.ajax({
        ...
        method: "GET"
            ...
            for(var i=0; i < 30; i++) {
                if(res.data[i].specialties[0].name !== 'Dentistry' && res.data[i].specialties[0].name !== 'Ophthalmology') {
                    if(primaryDoc && (res.data[i].specialties[0].name === 'Family Medicine' || res.data[i].specialties[0].name === 'Internal Medicine')) {
                        createDocCard(i, res);
                    } 
                    if(!primaryDoc && (res.data[i].specialties[0].name !== 'Family Medicine' || res.data[i].specialties[0].name !== 'Internal Medicine')) {
                        createDocCard(i, res);
